...
localparam TEXT_LENGTH2 = 112;
localparam TEXT_LENGTH3 = 122836;
parameter FILENAME_LENGTH = 20;

input [8*FILENAME_LENGTH-1:0] filename;

integer i;
integer plaintext2_ascii, plaintext3_ascii;

reg [6-1:0] in_dat2[0:TEXT_LENGTH2-1];
reg [6-1:0] in_dat3[0:TEXT_LENGTH3-1];
reg [7:0] ascii_code;

begin
  
  if(inputtext_num==0)begin
    $readmemh(filename,in_dat2);
    plaintext2_ascii = $fopen("plaintext2_ascii.dat");
    for(i=0;i<TEXT_LENGTH2;i=i+1) begin
      EnigmaCodetoASCII(in_dat2[i],ascii_code);
      $fwrite(plaintext2_ascii,"%s",ascii_code);
    end
  end
  
  else begin
    $readmemh(filename,in_dat3);
    plaintext3_ascii = $fopen("plaintext3_ascii.dat");
    for(i=0;i<TEXT_LENGTH3;i=i+1) begin
      EnigmaCodetoASCII(in_dat3[i],ascii_code);
      $fwrite(plaintext3_ascii,"%s",ascii_code);
    end
  end
